...
Fixtures used:
  isolated_client — TestClient with in-memory DB (get_db overridden)
...
def test_download_completed_project(isolated_client, test_db):
    ...
    try:
        resp = isolated_client.get(f"/api/v1/projects/{project_id}/download")
...
def test_download_filename(isolated_client, test_db):
...
def test_download_nonexistent_project(isolated_client):
    """Random UUID → 404."""
    resp = isolated_client.get(f"/api/v1/projects/{uuid.uuid4()}/download")
    assert resp.status_code == 404


def test_download_pending_project(isolated_client, test_db):
    """status=pending → 400."""
...
def test_download_processing_project(isolated_client, test_db):
...
def test_download_failed_project(isolated_client, test_db):
    """status=failed → 400."""
...
def test_download_no_output_dir(isolated_client, test_db):
