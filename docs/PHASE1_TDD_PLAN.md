...
Phase 1 目标（PRD §9）：**Transamo 内部 MVP** — 在 Phase 0 端到端流程（上传→处理→下载）基础上，叠加用户认证、多租户隔离、在线结果表格查看、参数配置表单、邮件通知，并完成基础设施从 SQLite/BackgroundTasks 迁移至 Supabase + Celery + R2。
...
## GROUP G：邮件通知（Task 21）

### Task 21：任务完成邮件

**创建文件**：`backend/app/services/email.py`

```python
def send_completion_email(to: str, project_id: str, project_name: str) -> None:
    # SMTP（开发）/ SendGrid API（生产）
    # 邮件内含项目直链：{FRONTEND_URL}/projects/{project_id}
```

新增 Settings 字段：`SMTP_HOST`、`SMTP_PORT`、`SMTP_USER`、`SMTP_PASSWORD`、`FRONTEND_URL`。

**修改文件**：`backend/app/services/worker.py`

在 Celery task 完成后调用 `send_completion_email(user.email, project_id, ...)`。

**测试**（`tests/test_email.py`，mock SMTP）：
1. `test_email_sent_on_completion`（mock `smtplib.SMTP`）— verify `sendmail` 被调用
2. `test_email_contains_project_link` — 邮件正文含 project_id
3. `test_email_not_sent_on_failure` — status=failed 不发送
...
| Sprint | 任务 | 目标 |
|--------|------|------|
| Sprint 1（基础设施）| Task 1–5 | Supabase + Alembic + Docker 就绪 |
| Sprint 2（认证）| Task 6–10 | 登录/注册可用，端点受保护 |
| Sprint 3（租户 + Celery）| Task 11–15 + Task 16 | 多租户隔离，异步队列运行 |
| Sprint 4（结果 + 邮件）| Task 17–21 | F-06 在线表格可查，邮件通知可用 |
...
4. 观察 7 步实时进度条
5. 处理完成 → 收到邮件通知（含项目链接）
6. 在线查看各结果表（A1–F4）：分页、排序、文本搜索
7. 下载单表 CSV（分号分隔，Excel 可直接打开）
8. 下载全量 ZIP

**Phase 1 成功标准（PRD §10）**：
> Transamo 业务分析人员无需技术指导，独立完成 3 次不同 GTFS 数据集的处理与结果查看。
