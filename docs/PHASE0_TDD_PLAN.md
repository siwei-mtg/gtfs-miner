...
**版本**：1.4
**日期**：2026-04-04
**状态**：后端全部完成（Sprint 1+2），前端待执行（Sprint 3+4）
...
| 前端 | **5%** | 仅 Vite + React 脚手架（默认 counter demo） |
...
**修改文件**：
- `frontend/vite.config.ts` — 添加 `server.proxy`：`/api` → `http://localhost:8000`（含 `ws: true`）
- `frontend/package.json` — 添加 devDeps：`vitest`、`@testing-library/react`、`@testing-library/jest-dom`、`jsdom`；添加 `"test": "vitest run"` script

**创建文件**：`frontend/src/__tests__/smoke.test.ts`（`1 + 1 === 2` 冒烟测试）

**验证方式**：`npm run test` 通过。
...
- `ProjectCreate`、`ProjectResponse`、`WebSocketMessage`、`UploadResponse`
- `ProjectStatus = "pending" | "uploading" | "processing" | "completed" | "failed"`

**验证方式**：`tsc --noEmit` 编译通过。
...
**创建文件**：
- `frontend/src/api/client.ts` — 函数：`createProject()`、`uploadGtfs()`、`getProject()`、`getDownloadUrl()`
- `frontend/src/__tests__/api.test.ts` — mock `fetch` 验证请求格式
...
**创建文件**：
- `frontend/src/hooks/useProjectProgress.ts` — `{ messages, latestStatus, isConnected }`
- `frontend/src/__tests__/useProjectProgress.test.ts` — mock WebSocket 验证连接/断开/消息更新
...
**创建文件**：
- `frontend/src/components/UploadForm.tsx` — 文件选择 + 参数配置 + 提交
- `frontend/src/__tests__/UploadForm.test.tsx` — 渲染、禁用状态、提交回调、加载态、错误显示
...
**创建文件**：
- `frontend/src/components/ProgressPanel.tsx` — 7 步进度 + 耗时 + 状态指示
- `frontend/src/__tests__/ProgressPanel.test.tsx` — 等待态、步骤文本、耗时、完成态、失败态

**设计**：纯展示组件，props 接收 `messages: WebSocketMessage[]`。
...
**创建文件**：
- `frontend/src/components/DownloadButton.tsx` — `<a href="/api/v1/projects/{id}/download" download>` 样式为按钮
- `frontend/src/__tests__/DownloadButton.test.tsx` — disabled/enabled 状态、href 正确性
...
**修改文件**：
- `frontend/src/App.tsx` — 替换 demo 为：状态机 `idle → uploading → processing → completed/failed`
- `frontend/src/App.css` — 最简样式
- `frontend/src/__tests__/App.test.tsx` — 初始态渲染、上传后切换视图、完成后显示下载
